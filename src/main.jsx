...
const getSession = () => {
try { return JSON.parse(localStorage.getItem("parure_session") || "null"); } catch { return
};
...
setLoading(false);
const users = getUsers();
const profile = { birthdate: form.birthdate, height: form.height, weight: form.weight,
...
if (!u || u.password !== form.password) { setErrors({ password: "ユーザー名またはパスワードが
...
{ label: "冬（ブルベ冬）", value: "winter", color: "#e8eaf6", desc: "はっきりとした鮮やかな色が得
];
if (done) {
return (
...
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "26px", fontWeight
...
<style>{`
@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&f
@keyframes fadeUp { from{opacity:0;transform:translateY(16px)} to{opacity:1;transform
input:focus { border-color: #9b59b6 !important; background: white !important; }
...
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "34px", fontWeight:
Parure
</div>
</div>
...
{/* Tab */}
<div style={{ display: "flex", background: "#f0f0f0", borderRadius: "14px", padding:
{["login", "register"].map((m) => (
<button key={m} onClick={() => { setMode(m); setStep(1); setErrors({}); setForm({
flex: 1, padding: "10px", border: "none", borderRadius: "11px",
fontWeight: "700", fontSize: "14px", cursor: "pointer", transition: "all background: mode === m ? "white" : "transparent",
...
<input placeholder="ユーザー名" value={form.name} onChange={set("name")} style={i
{errors.name && <div style={errStyle}>{errors.name}</div>}
<div>
<input placeholder="パスワード" type="password" value={form.password} onChange={s
{errors.password && <div style={errStyle}>{errors.password}</div>}
</div>
<button onClick={handleLogin} disabled={loading} style={{ ...btnStyle, opacity: l
{loading ? "処理中…" : "ログイン"}
</button>
</div>
)}
...
<div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
<div style={{ fontSize: "12px", color: "#aaa", textAlign: "center", marginBottom:
<div>
<input placeholder="ユーザー名（2文字以上）" value={form.name} onChange={set("name"
{errors.name && <div style={errStyle}>{errors.name}</div>}
</div>
<div>
</div>
<input placeholder="パスワード（4文字以上）" type="password" value={form.password}
{errors.password && <div style={errStyle}>{errors.password}</div>}
<div>
</div>
</div>
<input placeholder="パスワード（確認）" type="password" value={form.confirm} onCha
...
<div style={{ fontSize: "12px", color: "#aaa", textAlign: "center" }}>ステップ 2 /
{/* 生年月日 */}
<div>
<label style={{ fontSize: "12px", fontWeight: "700", color: "#555", display: "b
<input type="date" value={form.birthdate} onChange={set("birthdate")} style={in
{errors.birthdate && <div style={errStyle}>{errors.birthdate}</div>}
</div>
}}>
{/* 身長・体重 */}
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" <div>
<label style={{ fontSize: "12px", fontWeight: "700", color: "#555", display:
<input type="number" placeholder="例: 158" value={form.height} onChange={set(
{errors.height && <div style={errStyle}>{errors.height}</div>}
</div>
<div>
<label style={{ fontSize: "12px", fontWeight: "700", color: "#555", display:
<input type="number" placeholder="例: 52" value={form.weight} onChange={set("w
...
{PC_OPTIONS.map(pc => (
<button key={pc.value} onClick={() => setVal("personalColor", pc.value)} st
...
cursor: "pointer", textAlign: "left", fontFamily: "'Noto Sans JP', transition: "all 0.15s",
}}>
<div style={{ width: "36px", height: "36px", borderRadius: "50%", backgro
<div>
<div style={{ fontSize: "13px", fontWeight: "700", color: form.personal
<div style={{ fontSize: "11px", color: "#999", marginTop: "2px" }}>{pc.
</div>
</button>
sans-s
))}
...
<label style={{ fontSize: "12px", fontWeight: "700", color: "#555", display: "b
<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
{ value: "straight", label: "ストレート", desc: "筋肉質・立体的・上重心。シンプルで
{ value: "wave", label: "ウェーブ", desc: "華奢・曲線的・下重心。柔らかくふん
{ value: "natural", label: "ナチュラル", desc: "骨や関節が目立つ・フレーム感。ゆる
].map(sk => (
<button key={sk.value} onClick={() => setVal("skeletonType", sk.value)} sty
display: "flex", alignItems: "center", gap: "12px",
padding: "12px 14px", borderRadius: "14px", border: "2px solid",
...
fontSize: "16px", color: form.skeletonType === sk.value ? "white" : "#a
fontWeight: "700", transition: "all 0.15s",
}}>{sk.icon}</div>
<div>
<div style={{ fontSize: "13px", fontWeight: "700", color: form.skeleton
<div style={{ fontSize: "11px", color: "#999", marginTop: "2px" }}>{sk.
</div>
</button>
))}
</div>
...
<button onClick={handleRegister} disabled={loading} style={{ ...btnStyle, flex:
...
<button onClick={() => { saveSession({ name: "ゲスト", isGuest: true }); onLogin({ nam
...
const SAMPLE_ITEMS = [
{ id: 1, name: "白Tシャツ", category: "トップス", color: "#f5f5f5", emoji: " ", tags: ["カジュ
{ id: 2, name: "デニムジャケット", category: "アウター", color: "#4a6fa5", emoji: " { id: 3, name: "ブラックスキニー", category: "ボトムス", color: "#1a1a1a", emoji: " { id: 4, name: "フローラルワンピース", category: "ワンピース", color: "#e8a0bf", emoji: " { id: 5, name: "白スニーカー", category: "シューズ", color: "#ffffff", emoji: " { id: 6, name: "レザートート", category: "バッグ", color: "#8B5E3C", emoji: " { id: 7, name: "ストライプシャツ", category: "トップス", color: "#d0e8f5", emoji: " { id: 8, name: "プリーツスカート", category: "ボトムス", color: "#c9a0dc", emoji: " ", tags: [
", tags: [
", tag
", tags: ["カジ
", tags: ["きれい
...
const TRENDS = [
{ id: 1, title: "ゆるシルエット", desc: "オーバーサイズのアイテムで今季トレンドを取り入れて", tag: "202
{ id: 2, title: "モノトーンコーデ", desc: "白・黒・グレーでまとめたスタイリッシュな着こなし", tag: "定
{ id: 3, title: "デニムonデニム", desc: "同素材の重ね着がおしゃれ上級者の証", tag: "人気急上昇", col
{ id: 4, title: "バイオレットカラー", desc: "今季注目のパープル系カラーで個性を演出", tag: "2026SS",
{ id: 5, title: "レイヤードスタイル", desc: "重ね着でボリュームと深みを出す着こなし術", tag: "秋冬先取
{ id: 6, title: "Y2Kリバイバル", desc: "2000年代テイストで今どきにアップデート", tag: "トレンド", co
];
...
const WEATHER = { temp: "22°C", desc: "晴れ", icon: " ", advice: "日差しが強いので薄手の羽織を持参
...
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "20px", fontWeight:
{WEATHER.temp} {WEATHER.desc}
<div style={{ fontSize: "12px", color: "#8B5E3C", marginTop: "2px" }}>{WEATHER.advice
);
}
...
<span style={{ fontSize: "11px", color: "#2a7a2a", display: "flex", alignItems: "center
<span style={{ background: "rgba(255,255,255,0.85)", border: "1.5px solid rgba(60,180
クローゼットにあり
</span>
<span style={{ fontSize: "11px", color: "#a05010", display: "flex", alignItems: "center
<span style={{ background: "rgba(255,220,180,0.7)", border: "1.5px solid rgba(220,140
要購入
</span>
</div>
);
...
<h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "20px", margin: "8p
{coord.title}
</h3>
</div>
...
<div style={{ flexShrink: 0, background: "rgba(255,255,255,0.45)", borderRadius: "14p
<FashionAvatar profile={user?.profile} coordItems={coord.items} cardColor={coord.co
</div>
<div style={{ flex: 1 }}>
<div style={{ fontSize: "11px", fontWeight: "700", color: "#888", marginBottom: "6p
<ClosetLegend />
...
const bodyScale = bmi < 17 ? 0.82 : bmi < 19 ? 0.88 : bmi < 22 ? 1.0 : bmi < 26 ? 1.10 : 1.
// ── 座標定義 viewBox 80×300 ──────────────────────────────
const x = 40;
// 頭（固定）
...
const hairTop = `M${x-hRx-1} ${hY-2} Q${x-hRx-3} ${hY-hRy-3} ${x} ${hY-hRy-4}
Q${x+hRx+3} ${hY-hRy-3} ${x+hRx+1} ${hY-2} Z`;
const hairSideL = hLen > 0
? `M${x-hRx} ${hY+2} Q${x-hRx-6} ${hY+hLen*0.45} ${x-hRx-4} ${hY+hLen}
Q${x-hRx-1} ${hY+hLen+5} ${x-hRx+5} ${hY+hLen}
Q${x-hRx-1} ${hY+hLen*0.45} ${x-hRx+2} ${hY+2} Z`
: null;
...
const bodyBase = `M${x-nkW} ${nkB} C${x-shW} ${shY-2} ${x-buW-2} ${buY-4} ${x-waW} ${waY}
L${x-waW} ${waY} L${x+waW} ${waY}
C${x+buW+2} ${buY-4} ${x+shW} ${shY-2} ${x+nkW} ${nkB} Z`;
// トップス（衿元～裾・前身頃のみ・シンプルに）
const topFill = `M${x-nkW} ${nkB+2}
L${x-shW} ${shY+1}
C${x-buW-2} ${buY-4} ${x-waW} ${waY-6} ${x-waW} ${waY}
L${x+waW} ${waY}
C${x+waW} ${waY-6} ${x+buW+2} ${buY-4} ${x+shW} ${shY+1}
L${x+nkW} ${nkB+2}
Q${x} ${nkB} ${x-nkW} ${nkB+2} Z`;
// 袖の長さを判定
const isNoSleeve = (coordItems || []).some(i => /ノースリーブ|キャミ|チューブ/.test(i));
const isShortSleeve = !isNoSleeve && (coordItems || []).some(i => /半袖|Tシャツ/.test(i) && !
const isSevenSleeve = !isNoSleeve && !isShortSleeve && (coordItems || []).some(i => /七分袖/
const sleeveEndY = isNoSleeve ? shY + 2
: isShortSleeve ? shY + 14
: isSevenSleeve ? shY + 30
: waY;
const sleeveL = isNoSleeve ? null : `M${x-shW} ${shY+1}
C${x-shW-5} ${shY+4} ${x-shW-7} ${sleeveEndY-4} ${x-shW-5} ${sleeveEndY}
L${x-shW-1} ${sleeveEndY+1}
C${x-shW} ${sleeveEndY-2} ${x-shW} ${shY+5} ${x-shW+2} ${shY+3} Z`;
const sleeveR = isNoSleeve ? null : `M${x+shW} ${shY+1}
C${x+shW+5} ${shY+4} ${x+shW+7} ${sleeveEndY-4} ${x+shW+5} ${sleeveEndY}
L${x+shW+1} ${sleeveEndY+1}
C${x+shW} ${sleeveEndY-2} ${x+shW} ${shY+5} ${x+shW-2} ${shY+3} Z`;
// アウター本体（インナーを完全に覆う・不透明）
const outerBody = `M${x-nkW-1} ${nkB+1}
L${x-shW-3} ${shY}
C${x-buW-5} ${buY-3} ${x-waW-3} ${waY-4} ${x-waW-2} ${waY+22}
L${x+waW+2} ${waY+22}
C${x+waW+3} ${waY-4} ${x+buW+5} ${buY-3} ${x+shW+3} ${shY}
L${x+nkW+1} ${nkB+1}
Q${x} ${nkB-1} ${x-nkW-1} ${nkB+1} Z`;
// アウター袖（大きめ・インナー袖を完全カバー）
const outerSleeveL = `M${x-shW-3} ${shY}
C${x-shW-10} ${shY+6} ${x-shW-13} ${buY+6} ${x-shW-10} ${waY+10}
L${x-shW-4} ${waY+13}
C${x-shW-4} ${buY+4} ${x-shW-2} ${shY+7} ${x-shW+1} ${shY+3} Z`;
const outerSleeveR = `M${x+shW+3} ${shY}
C${x+shW+10} ${shY+6} ${x+shW+13} ${buY+6} ${x+shW+10} ${waY+10}
L${x+shW+4} ${waY+13}
C${x+shW+4} ${buY+4} ${x+shW+2} ${shY+7} ${x+shW-1} ${shY+3} Z`;
// スカート（シンプルAライン）
const skirtFill = `M${x-waW-1} ${waY}
L${x-hiW-12} ${anY-6}
L${x+hiW+12} ${anY-6}
L${x+waW+1} ${waY} Z`;
const skirtHem = `M${x-hiW-12} ${anY-6} Q${x} ${anY} ${x+hiW+12} ${anY-6}`;
// ワンピース（肩～Aライン裾）
const dressFill = `M${x-nkW} ${nkB+2}
L${x-shW} ${shY+1}
C${x-buW-2} ${buY-4} ${x-waW} ${waY-6} ${x-hiW-12} ${anY-6}
L${x+hiW+12} ${anY-6}
C${x+waW} ${waY-6} ${x+buW+2} ${buY-4} ${x+shW} ${shY+1}
L${x+nkW} ${nkB+2}
Q${x} ${nkB} ${x-nkW} ${nkB+2} Z`;
...
// ヒップ～足首をひとつの閉じたパスで
const pantsBase = `M${x-waW} ${waY}
L${x-hiW} ${hiY}
C${x-hiW} ${hiY+6} ${x-anW-5} ${anY-8} ${x-anW-4} ${anY}
L${x+anW+4} ${anY}
C${x+anW+5} ${anY-8} ${x+hiW} ${hiY+6} ${x+hiW} ${hiY}
L${x+waW} ${waY} Z`;
// 股の割れ目線
...
? `M${x+shW+5} ${waY+4} L${x+shW+17} ${waY+4} Q${x+shW+19} ${waY+4} ${x+shW+19} ${waY+6}
L${x+shW+19} ${waY+22} Q${x+shW+19} ${waY+24} ${x+shW+17} ${waY+24}
L${x+shW+5} ${waY+24} Q${x+shW+3} ${waY+24} ${x+shW+3} ${waY+22}
L${x+shW+3} ${waY+6} Q${x+shW+3} ${waY+4} ${x+shW+5} ${waY+4} Z`
: null;
...
<path d={`M${x-waW} ${waY} L${x+waW} ${waY}`} stroke={lc} strokeWidth={sw} fill="no
</>
)}
{/* ── アウター（インナーを完全に覆う）── */}
{hasOuter && (
...
<line x1={x} y1={nkB+4} x2={x} y2={waY+20} stroke={lc} strokeWidth={0.6} opacity="0
</>
)}
{/* ── バッグ ── */}
{bagItem && bagPath && (
...
const HAIR_LABEL = { ショート: "ショートヘア", ミディアム: "ミディアムヘア", ロング: "ロングヘア", ベリー
const SITUATIONS = ["デート", "オフィス", "カジュアルお出かけ", "友達とランチ", "買い物", "旅行", "特別
const WEATHERS = [
{ label: " { label: " { label: " { label: " { label: " 晴れ", value: "晴れ、気温22度", temp: "22°C" },
曇り", value: "曇り、気温18度", temp: "18°C" },
雨", value: "雨、気温16度", temp: "16°C" },
寒い", value: "晴れ、気温8度の寒い日", temp: "8°C" },
暑い", value: "晴れ、気温32度の猛暑日", temp: "32°C" },
];
const DEMO_RESULTS = [
{
title: "春色カフェデート",
mood: "フェミニン",
items: ["コーラルピンクの半袖ブラウス", "アイボリーのフレアスカート", "ベージュのサンダル", "ミニカゴバ
colorPoint: "イエベ春の得意なコーラルピンクをトップスに取り入れ、アイボリーで明るくまとめました。肌のツヤ
tip: "158cmの身長を活かすためスカート丈はひざ下10cmが◎。バッグは小ぶりにして視線を上に引き上げて。",
reason: "身長158cm・イエベ春タイプに最適な明るいトーンのコーデです。ロングヘアとフレアスカートの組み合わせ
},
{
title: "こなれカジュアル",
mood: "カジュアル",
...
tip: "ニットをデニムにタックインするとウエスト位置が高く見えてスタイルアップ。スニーカーは白でまとめてすっき
...
colorPoint: "ピーチカラーはイエベ春の最得意色。ワンピース1枚でコーデが完成し、きれいめな印象を簡単に演出で
...
- 骨格タイプ: ${{ straight: "ストレート（シンプルでフィット感のある服が得意）", wave: "ウェーブ（ふんわり柔
...
max_tokens: 1500,
system: "あなたはプロのパーソナルスタイリストです。ユーザーの体型・パーソナルカラー・髪型を深く理解
...
setResults([{ title: "エラー", mood: "-", items: [], colorPoint: "", tip: "もう一度お試しく
}
setLoading(false);
};
...
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "18px", fontWeight:
AIパーソナルスタイリスト
</div>
{hasProfile ? (
<div style={{ fontSize: "11px", opacity: 0.85 }}>
{PC_LABEL[profile.personalColor]} · {profile.height}cm · {HAIR_LABEL[profile.hair
</div>
) : (
<div style={{ fontSize: "11px", opacity: 0.75 }}>登録するとよりパーソナルな提案が届きます<
)}
</div>
{/* 天気選択 */}
...
color: "white", fontSize: "15px", fontWeight: "700", cursor: loading ? "default" : "p
fontFamily: "'Noto Sans JP', sans-serif",
...
return (
<div style={{ background: color, borderRadius: "20px", padding: "20px", boxShad
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "
<div>
<span style={{ background: "rgba(255,255,255,0.8)", borderRadius: "20px",
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "20px",
</div>
</div>
{/* アバター＋アイテムリスト横並び */}
<div style={{ display: "flex", gap: "12px", alignItems: "flex-start", marginB
<div style={{ flexShrink: 0, background: "rgba(255,255,255,0.5)", borderRad
<FashionAvatar profile={user?.profile} coordItems={r.items} cardColor={co
</div>
<div style={{ flex: 1 }}>
<div style={{ fontSize: "11px", fontWeight: "700", color: "#888", marginB
<ClosetLegend />
<div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
{r.items.map((item, i) => (
<ItemTag key={i} label={item} inCloset={isInCloset(item, closetItems)
...
{r.colorPoint && (
<div style={{ background: "rgba(255,255,255,0.65)", borderRadius: "12px", p
<span style={{ fontWeight: "700", color: moodColor }}> カラーポイント</sp
</div>
)}
<div style={{ background: "rgba(255,255,255,0.65)", borderRadius: "12px", pad
<span style={{ fontWeight: "700" }}> スタイリングのコツ</span><br />{r.tip}
</div>
{r.reason && (
<div style={{ background: "rgba(255,255,255,0.65)", borderRadius: "12px", p
<span style={{ fontWeight: "700" }}> あなたに合う理由</span><br />{r.reaso
</div>
)}
</div>
);
...
const CATEGORY_OPTIONS = ["トップス", "ボトムス", "スカート", "アウター", "ワンピース", "シューズ", "バ
const EMOJI_OPTIONS = [" "," "," "," "," "," "," "," "," "," "," "," "," ","
const COLOR_OPTIONS = [
...
const TAG_OPTIONS = ["カジュアル", "きれいめ", "フェミニン", "モード", "スポーツ", "オフィス", "デート"
const BLANK_FORM = { name: "", category: "トップス", emoji: " ", color: "#f5f5f5", tags: [],
// カテゴリ別アイテム名選択肢
const ITEM_PRESETS = {
"トップス": ["Tシャツ", "カットソー", "ブラウス", "シャツ", "ニット", "スウェット", "タンクトップ",
"ボトムス": ["デニム", "スキニーパンツ", "ワイドパンツ", "スラックス", "ショートパンツ", "チノパン", "
"アウター": ["デニムジャケット", "テーラードジャケット", "ブルゾン", "トレンチコート", "ウールコート",
"ワンピース": ["シャツワンピース", "フレアワンピース", "タイトワンピース", "ニットワンピース", "マキシワンピ
"スカート": ["フレアスカート", "プリーツスカート", "タイトスカート", "ミニスカート", "マキシスカート",
"シューズ": ["スニーカー", "パンプス", "ローファー", "ブーツ", "サンダル", "ミュール", "バレエシューズ
"バッグ": ["トートバッグ", "ショルダーバッグ", "リュック", "クラッチバッグ", "ミニバッグ", "かごバッ
"アクセサリー": ["ネックレス", "ピアス", "イヤリング", "ブレスレット", "リング", "スカーフ", "ベルト",
"その他": ["ルームウェア", "水着", "レインウェア", "スポーツウェア", "ジャージ", "レオタード", "オー
};
...
const toggleTag = (t) => setF("tags", form.tags.includes(t) ? form.tags.filter(x => x !== t
...
<style>{`@keyframes slideUp { from{transform:translateY(100%)} to{transform:translate
<div style={{ width: "40px", height: "4px", background: "#e0e0e0", borderRadius: "4px
...
{/* Preview */}
<div style={{ display: "flex", alignItems: "center", gap: "14px", background: "#f7f5f
<div style={{
...
<div>
<div style={{ fontWeight: "700", fontSize: "15px", color: "#2a2a2a" }}>{fullName
<div style={{ fontSize: "12px", color: "#aaa", marginTop: "2px" }}>{form.category
<div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginTop: "4px" }}>
{form.tags.map((t, i) => <span key={i} style={{ background: "#eee", borderRadiu
</div>
</div>
</div>
{/* Inner tabs */}
<div style={{ display: "flex", background: "#f0f0f0", borderRadius: "12px", padding:
...
<button key={cat} onClick={() => { setF("category", cat); setF("name", "");
padding: "7px 14px", borderRadius: "20px", border: "2px solid",
...
<button key={s.value} onClick={() => setF("sleeveLength", form.sleeveLeng
padding: "7px 14px", borderRadius: "20px", border: "2px solid",
...
<label style={{ fontSize: "12px", fontWeight: "700", color: "#555" }}>アイテム名
<div style={{ display: "flex", background: "#f0f0f0", borderRadius: "10px", p
{[["preset","選択"], ["custom","手入力"]].map(([m, l]) => (
<button key={m} onClick={() => setNameMode(m)} style={{
padding: "4px 10px", border: "none", borderRadius: "8px", fontSize: "11
...
<div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
{COLOR_OPTIONS.map(c => (
<button key={c.value} onClick={() => setF("color", c.value)} title={c.label
...
{tab === "tags" && (
<div>
<label style={{ fontSize: "12px", fontWeight: "700", color: "#555", display: "blo
<div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
{TAG_OPTIONS.map(t => (
...
<button onClick={() => canSave && onSave({ ...form, name: fullName || form.name })} s
...
{/* Category filter */}
<div style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "8px", mar
{categories.map(cat => (
...
<div style={{ fontSize: "13px", fontWeight: "600", color: "#2a2a2a", marginBottom
<div style={{ fontSize: "11px", color: "#aaa" }}>{item.category}</div>
<div style={{ display: "flex", gap: "4px", flexWrap: "wrap", justifyContent: "cen
{item.tags.map((tag, i) => (
<span key={i} style={{ background: "#f5f5f5", borderRadius: "8px", padding: "
))}
</div>
</div>
))}
...
display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "ce
minHeight: "120px",
}}>
...
category: "総合ファッション",
sites: [
{ name: "ZOZOTOWN", desc: "国内最大級のファッション通販", emoji: " { name: "UNIQLO", desc: "ベーシックアイテムならここ", emoji: " { name: "GU", desc: "トレンドをお手頃価格で", emoji: " { name: "H&M", desc: "世界的人気のファストファッション", emoji: " ", color: "#000000", url
", color: "#e60012", url: "h
", color: "#6a1b9a", url: "https://
", color: "#cc0000", url:
]
},
{
category: "セレクトショップ",
sites: [
{ name: "BEAMS", desc: "トレンドセンスあふれるセレクト", emoji: " { name: "SHIPS", desc: "上質なきれいめカジュアル", emoji: " { name: "UNITED ARROWS", desc: "こだわりの大人スタイル", emoji: " { name: "Rakuten Fashion", desc: "ブランドが豊富なファッション通販", emoji: " ", color: "#2e4a7a", url:
", color: "#1a3a5c", url: "htt
", color: "#2a2a2a", url
", color: "#b
]
},
...
sites: [
{ name: "FARFETCH", desc: "世界中のラグジュアリーを一括購入", emoji: " { name: "NET-A-PORTER", desc: "世界トップクラスのラグジュアリー通販", emoji: " { name: "SSENSE", desc: "ストリート×ラグジュアリーの融合", emoji: " ", color: "#222222",
", color: "#1
", color: "#000000", ur
]
},
...
category: "フリマ・中古",
sites: [
{ name: "メルカリ", desc: "フリマアプリの定番", emoji: " ", color: "#ff0211", url: "https:
{ name: "FRIL（フリル）", desc: "レディースに特化したフリマ", emoji: " ", color: "#e91e8c",
{ name: "ラクマ", desc: "楽天のフリマアプリ", emoji: " ", color: "#009900", url: "https://
]
},
];
...
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "20px", fontWeight:
<div style={{ fontSize: "13px", opacity: 0.85, marginTop: "4px" }}>お気に入りのサイトへジ
</div>
{/* Search */}
<div style={{ position: "relative", marginBottom: "20px" }}>
<span style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY
<input
value={search}
...
<div style={{ fontSize: "13px", fontWeight: "700", color: "#888", marginBottom: "10
{cat.category}
</div>
...
onMouseEnter={e => { e.currentTarget.style.transform = "translateX(4px)"; e
onMouseLeave={e => { e.currentTarget.style.transform = "translateX(0)"; e.c
>
<div style={{
...
<div style={{ fontWeight: "700", fontSize: "15px", color: "#2a2a2a" }}>{s
<div style={{ fontSize: "12px", color: "#aaa", marginTop: "2px" }}>{site.
</div>
...
? `身長${profile.height}cm・体重${profile.weight}kg・${profile.hairLength}ヘア・パーソナルカ
: "プロフィール未登録";
const favoriteStyles = (user?.favoriteStyles || [])
.map(k => STYLE_OPTIONS.find(s => s.key === k)?.label)
.filter(Boolean).join("・") || "指定なし";
const prompt = `トレンドキーワード「${trend.title}」（${trend.desc}）をテーマにしたコーデを10パター
ユーザー情報:
- ${profileDesc}
...
"title": "コーデ名（10文字以内）",
"mood": "雰囲気タグ",
"items": ["アイテム1（トップス・ワンピース・アウターには半袖/長袖/七分袖/ノースリーブを明記）", "アイテム
...
model: "claude-sonnet-4-20250514",
max_tokens: 2000,
system: "あなたはトレンドに精通したファッションスタイリストです。指定されたトレンドキーワードに沿っ
...
{/* トレンドカードグリッド */}
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBotto
{TRENDS.map(trend => {
...
<div style={{ fontSize: "14px", fontWeight: "700", color: "#2a2a2a", marginBott
<div style={{ fontSize: "11px", color: "#666", lineHeight: "1.5", marginBottom:
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "ce
<span style={{
...
<div style={{ fontSize: "32px", marginBottom: "12px", animation: "spin 1s linear in
...
justifyContent: "center", fontSize: "12px", fontWeight: "700", flexShrink:
}}>{i + 1}</div>
<div>
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "16px", fo
<span style={{
...
const STYLE_OPTIONS = [
{ key: "stylish", label: "スタイリッシュ", emoji: " ", desc: "クール・モード・洗練", col
{ key: "girly", label: "ガーリー", emoji: " ", desc: "フェミニン・甘め・可愛い", co
{ key: "casual", label: "カジュアル", emoji: " ", desc: "ラフ・リラックス・デイリー", co
{ key: "natural", label: "ナチュラル", emoji: " ", desc: "オーガニック・アース・癒し", co
{ key: "office", label: "オフィス", emoji: " ", desc: "きれいめ・通勤・上品", co
{ key: "street", label: "ストリート", emoji: " ", desc: "ヴィンテージ・アメカジ", co
{ key: "romantic", label: "ロマンティック", emoji: " ", desc: "エレガント・上品・華やか", col
{ key: "sporty", label: "スポーティ", emoji: " ", desc: "アクティブ・機能的・元気", col
{ key: "minimal", label: "ミニマル", emoji: " ", desc: "シンプル・モノトーン・無駄なし",
{ key: "vintage", label: "ヴィンテージ", emoji: " ", desc: "レトロ・クラシック・個性的", co
];
const PC_LABEL_MAP = { spring: "イエベ春", summer: "ブルベ夏", autumn: "イエベ秋", winter: const HAIR_LABEL_MAP = { ショート: "ショート", ミディアム: "ミディアム", ロング: "ロング", ベリーロング:
"ブルベ冬
...
{ label: "お気に入り", value: likedCount, icon: " " },
...
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "22px", fontWeight:
{user?.name}
</div>
{selected.length > 0 && (
<div style={{ fontSize: "13px", color: "#9b59b6", marginTop: "6px", fontWeight: "60
...
<div style={{ fontSize: "20px", fontWeight: "700", color: "#2a2a2a" }}>{s.value}<
...
<div style={{ fontWeight: "700", fontSize: "14px", marginBottom: "12px", color: "#5
<div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
{age && (
...
{profile.hairLength && (
<span style={tagStyle("#e91e63")}>{HAIR_LABEL_MAP[profile.hairLength] || profil
)}
)}
{profile.skeletonType && (
<span style={tagStyle("#667eea")}>骨格{ { straight: "ストレート", wave: "ウェーブ",
...
<div style={{ fontWeight: "700", fontSize: "15px", marginBottom: "6px" }}> 好きなスタ
<div style={{ fontSize: "12px", color: "#aaa", marginBottom: "16px" }}>複数選択OK・AIコ
<div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
{STYLE_OPTIONS.map(s => {
...
<div style={{ flex: 1 }}>
<div style={{ fontWeight: "700", fontSize: "14px", color: isOn ? s.color :
{s.label}
</div>
<div style={{ fontSize: "11px", color: "#aaa", marginTop: "2px" }}>{s.desc}
</div>
<div style={{
...
{isOn && <span style={{ color: "white", fontSize: "13px", lineHeight: 1 }}>
</div>
</button>
);
})}
...
<span style={{ fontSize: "13px", color: "#9b59b6", fontWeight: "600" }}> <button onClick={() => { clearSession(); setUser(null); }} style={{
...
{new Date().toLocaleDateString("ja-JP", { month: "long", day: "numeric", weekday: "
</div>
</div>
}}>
{/* Content */}
<div style={{ flex: 1, padding: "20px", overflowY: "auto", paddingBottom: "80px" {activeTab === 0 && (
<div>
<WeatherBadge />
<AICoordSection user={user} closetItems={items} />
<div style={{ fontFamily: "'Playfair Display', serif", fontSize: "18px", fontWeig
おすすめコーデ
</div>
{COORD_SUGGESTIONS.map(coord => (
<CoordCard key={coord.id} coord={coord} onLike={toggleLike} liked={likedCoords[
))}
</div>
)}
{activeTab === 1 && <ClosetTab items={items} onAddItem={addItem} onEditItem={editItem
{activeTab === 2 && <TrendTab user={user} />}
{activeTab === 3 && <ShoppingTab />}
{activeTab === 4 && <ProfileTab user={user} onUpdateUser={updateUser} items={items} l
</div>
{/* Bottom Nav */}
<div style={{
...
<span style={{ fontSize: "20px", opacity: activeTab === i ? 1 : 0.4 }}>{tabIcons[
<span style={{
fontSize: "10px",
fontWeight: "600",
